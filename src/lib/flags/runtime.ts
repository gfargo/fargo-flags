...
export type ClientFlags = { [K in keyof ClientSchemaMap]: z.infer<ClientSchemaMap[K]> };
...
export async function resolveAllFlags(ctx?: FlagContext): Promise<Flags> {
  ...
  const entries = await Promise.all(
    keys.map(async (key) => {
      ...
      const raw = await Promise.resolve(def.decide?.(ctx as any) ?? def.defaultValue);
...
export function pickClientFlags(flags: Flags): ClientFlags {
  ...
  for (const key of clientFlagKeys) {
    ...
    out[key as string] = sanitize ? sanitize((flags as any)[key]) : (flags as any)[key];
  }
